...
requirejs(["SelectImageDialog",
           ...
           "kindeditor",
           "zh_lang"
],function(SelectImageDialog,UploadImageDialog,CityPickerMulti) {
	 $(document).ready(function(){    
       ...
       $("#button_UploadPic").click(function(){
       	   var uploader=new UploadImageDialog({
           	  handler:function(url){
           	  	  editor.insertHtml("<img src='"+url+"' width='100%'/>");
           	  }
           });
           uploader.show();
       });
       ...
       $("#saveDocumentButton").click(function(){
       	   ...
       	   $.ajax({
            	type: "POST",
              dataType: "json",
              url:"/advertisement/createDocument",
              data: doc,
              success: function(data){
              	  if(data.DocumentID){
              	  	  window.top.window.setTab("createDocument",{
              	  	      id:"myDocument_"+data.DocumentID,
              	  	      title:doc.title,
              	  	      url:"/advertisement/myDocument.jsp?DocumentID="+data.DocumentID	
              	  	  });
       ...
       function setBottomMessage(title,dom){
       	   $("#dialog_title").html(title);
       	   $("#dialog_main").empty();
       	   $("#dialog_main").append(dom);
       	   $("#popupButton").click();	 
       }
